...
import pandas as pd
#PARA FAZER: ver o nome dos ataques! pra anotar na variavel label_names
...
if __name__ == "__main__":
    ...
    print(f"INICIO TESTE RePO FLOW BASED ADVERSARIAL NovoDataset\n{time.strftime('%d-%m-%Y %H:%M:%S', time.localtime())}\n")
    ...
    label_names=['LDAP','NetBIOS','MSSQL','Portmap','Syn','UDP','UDPLag'] #do novo dataset
    
    ##attack_type = 'DoS Slowhttptest' #esse cara tem 5499 amostras somente no dataset antigo. E DEMOROU MUITO!!!
    
    attack_type='UDPLag' #aqui tem 1873 no total.
    ...
    print(f"FIM TESTE RePO FLOW BASED ADVERSARIAL NovoDataset\nTempo decorrido:{(time.time()-tempoInicial)}seg\n{time.strftime('%d-%m-%Y %H:%M:%S', time.localtime())}")